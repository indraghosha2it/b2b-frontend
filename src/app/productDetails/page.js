...
//         {/* Removed overflow-x-auto and negative margins to prevent scrollbar */}
...
// // Description Component - Responsive
...
//       <div className="p-4 sm:p-6">
//         <div 
//           className="prose prose-sm sm:prose max-w-none text-gray-600 leading-relaxed text-sm sm:text-base"
...
// // Inquiry Item Component - Shows ALL sizes
...
//   // Helper function for tag styling (define this outside or import)
...
//     queryParams.append('limit', 8); // You're fetching 8 products
...
//       setRelatedProducts(shuffled.slice(0, 12)); // BUT HERE YOU'RE SLICING TO ONLY 4 PRODUCTS
//     }
//   } catch (error) {
...
//   // Get first 5 colors to display
//   const colorsToShow = product.colors?.slice(0, 5) || [];
//   const remainingColors = product.colors?.length > 5 ? product.colors.length - 5 : 0;

//   // Get first 5 sizes to display
//   const sizesToShow = product.sizes?.filter(s => s.trim()).slice(0, 5) || [];
//   const remainingSizes = product.sizes?.filter(s => s.trim()).length > 5 ? product.sizes.filter(s => s.trim()).length - 5 : 0;
...
//                     <span className="px-2 sm:px-3 py-1 bg-[#E39A65] text-white text-xs sm:text-sm rounded-full">
//                       {product.category?.name || 'Uncategorized'}
//                     </span>
//                     {product.targetedCustomer && product.targetedCustomer !== 'unisex' && (
...
//                   <div 
//                     className="text-xs sm:text-sm text-gray-600 line-clamp-2 prose prose-sm max-w-none"
//                     dangerouslySetInnerHTML={{ 
//                       __html: product.description || 'No description available.' 
//                     }}
//                   />
...
//       ))}
//     </div>
//     {/* Show count if more than 15 colors (optional) */}
...
//       ))}
//     </div>
//     {/* Show count if more than 15 sizes (optional) */}
...
//                 {['attributes', 'description', 'pricing', 'shipping', 'reviews'].map((tab) => (
//                   <button
//                     key={tab}
...
//                 <div>
//                   <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900">Related Products</h2>
//                   <p className="text-xs sm:text-sm text-gray-500 mt-1">You might also be interested in these products</p>
//                 </div>
//                 <Link 
//                   href="/products" 
//                   className="inline-flex items-center justify-center gap-1 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-[#E39A65] text-white rounded-lg hover:bg-[#d48b54] transition-colors text-xs sm:text-sm font-medium group w-full sm:w-auto"
//                 >
//                   View All Products
...
//       {/* Products Grid with Animation - Added padding to prevent button overlap */}
