...
// ✅ FIX: Sync editor with formData.content when it changes (e.g., after fetching)
useEffect(() => {
...
// Also add a small delay to ensure editor is ready
useEffect(() => {
  if (editor && !isLoading && formData.content) {
    // Small timeout to ensure editor is fully initialized
  ...
  const handleFeaturedImageChange = (e) => {
    ...
    reader.onloadend = () => {
      setFeaturedImage({
        ...
        error: '',
        existingUrl: null, // Clear existing URL when new file is selected
        existingPublicId: null
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (existingThumbnails.length > 0) {
        ...
        console.log('Sending thumbnails to keep:', thumbnailsToKeep); // Debug log
      } else {
