...
// //             {/* Product Image */}
// //             <div className="w-16 h-[88px] bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg border border-gray-200 overflow-hidden shadow-sm flex-shrink-0">
...
// //   const handleEdit = () => {
// //     router.push(`/admin/invoices/edit?invoiceId=${invoiceId}`);
// //   };
...
// //                 >
// //                   {updating ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Change Status'}
...
// //                 {isExpired && ` (${overdueDays} days overdue)`}
// //               </span>
// //             </div>
...
// //                   <span>Inquiry: {invoice.inquiryNumber}</span>
// //                 </div>
// //               </>
// //             )}
// //           </div>
...
// //         {invoice.customer?.email && ` Please contact ${invoice.customer.email} for payment.`}
// //       </span>
// //     </div>
// //   )}
...
// // FIXED: Check if invoice is expired (due date passed)
// const isInvoiceExpired = (invoice) => {
//   const today = new Date();
//   const dueDate = new Date(invoice.dueDate);
...
//                       <span className={isExpired ? 'text-red-600 font-medium' : ''}>
//                         Due: {formatDate(invoice.dueDate)}
//                         {isExpired && ` (${overdueDays} day${overdueDays !== 1 ? 's' : ''} overdue)`}
//                       </span>
//                     </div>
//                     {invoice.inquiryNumber && (
//                       <>
    ...
    <div className="min-h-screen bg-gray-50 py-6">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-6">
          ...
          {/* Overdue Warning - Shows only when invoice is expired */}
          {isExpired && (
            <div className="px-6 py-3 bg-red-50 border-b border-red-200 flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-red-600" />
              <span className="text-sm text-red-700">
