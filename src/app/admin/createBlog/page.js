...
//   // Sync editor with paragraph description - FIXED with proper dependency array
//   useEffect(() => {
//     if (!editor) return;

//     const currentHTML = editor.getHTML();
//     const newHTML = paragraph.description || '';

//     if (currentHTML !== newHTML) {
//       editor.commands.setContent(newHTML, false);
//     }
//   }, [editor, paragraph.description]); // ✅ Added dependency array
...
//     if (!allowedExtensions.includes(fileExtension)) {
//       onUpdate(sectionIndex, 'imageError', `Invalid format: .${fileExtension}. Allowed: ${allowedExtensions.join(', ')}`);
//       return;
//     }

//     if (file.size > 5 * 1024 * 1024) {
//       const fileSizeMB = (file.size / (1024 * 1024)).toFixed(2);
//       onUpdate(sectionIndex, 'imageError', `File too large: ${fileSizeMB}MB. Max: 5MB`);
//       return;
//     }

//     // Use FileReader exactly like thumbnail images
//     const reader = new FileReader();
//     reader.onloadend = () => {
//       onUpdate(sectionIndex, 'imagePreview', reader.result);
//       onUpdate(sectionIndex, 'imageFile', file);
//       onUpdate(sectionIndex, 'imageError', '');
//     };
//     reader.readAsDataURL(file);
//   };

//   const removeImage = () => {
//     onUpdate(index, 'imagePreview', null);
//     onUpdate(index, 'imageFile', null);
//     onUpdate(index, 'imageError', '');
//     if (paragraphImageRef.current) {
//       paragraphImageRef.current.value = '';
//     }
//   };
...
//         {/* Section Image - EXACT copy of thumbnail image pattern */}
//         <div>
//           <label className="block text-sm font-medium text-gray-600 mb-1.5">
...
//                 paragraph.imageError ? 'border-red-300 bg-red-50' : 'border-gray-300 hover:border-[#E39A65] hover:bg-orange-50'
//               }`}
...
//               />
//               <Upload className={`w-6 h-6 mx-auto mb-1 ${paragraph.imageError ? 'text-red-400' : 'text-gray-400'}`} />
//               <p className={`text-xs ${paragraph.imageError ? 'text-red-600' : 'text-gray-500'}`}>
...
//           {paragraph.imageError && (
//             <p className="text-xs text-red-600 mt-2">{paragraph.imageError}</p>
//           )}
//         </div>
...
//     extensions: [
//       StarterKit.configure({
//         link: false, // Disable built-in link to avoid duplication
//       }),
//       TipTapLink.configure({
//         openOnClick: false,
//         HTMLAttributes: {
...
//           header: p.header,
//           description: p.description
//           // Don't include image here - it will be sent as separate files
//         }));
...
//                   </h2>
//                   <p className="text-xs text-gray-500 mt-1">
//                     Add more sections with header, image, and rich text description
//                   </p>
//                 </div>
